...
public class IRGen {
  ...
  private static ClassInfo createClassInfo(Ast.ClassDecl n) throws Exception {
    ...
    int size = classInfo.objSize;
    for(Ast.VarDecl v: n.flds)
    {
        classInfo.offsets.put(v.nm, size);
        size += gen(v.t).size;
    }
    classInfo.objSize = size;
    return classInfo;
  }
  ...
  static List<IR.Func> gen(Ast.ClassDecl n, ClassInfo cinfo) throws Exception {
    ...
    List<IR.Func> methods = new ArrayList<IR.Func>();
    for(Ast.MethodDecl m: n.mthds)
        methods.add(gen(m, cinfo));
  ...
  static IR.Func gen(Ast.MethodDecl n, ClassInfo cinfo) throws Exception {
    ...
    IR.Global label = null;  
    if(n.nm.equals("main") == false){
        ClassInfo parentInfo = cinfo.methodBaseClass(n.nm);
        label = new IR.Global("_"+parentInfo.name+"_"+n.nm);
        params.add(thisObj);
    }
    else
        label = new IR.Global("_"+n.nm);
    Env env = new Env();
  ...
  static List<IR.Inst> gen(Ast.VarDecl n, ClassInfo cinfo, 
    ...
    List<IR.Inst> code = new ArrayList<IR.Inst>();
    IR.Id toAdd = new IR.Id(n.nm);
    if(n.init != null){
        CodePack var = gen(n.init, cinfo, env);
        code.addAll(var.code);
        code.add(new IR.Move(toAdd, var.src));
    }
    return code;
  ...
  static List<IR.Inst> gen(Ast.Assign n, ClassInfo cinfo, Env env) throws Exception {
      List<IR.Inst> code = new ArrayList<IR.Inst>();
      CodePack rhsGen = gen(n.rhs, cinfo, env);
      code.addAll(rhsGen.code);
      CodePack lhsGen;
      if(n.lhs instanceof Ast.Id){
        lhsGen = gen(n.lhs, cinfo, env);
        
        if(env.containsKey(((Ast.Id)n.lhs).nm))
            code.add(new IR.Move((IR.Id)lhsGen.src, rhsGen.src));

        else{
            Ast.Field field = new Ast.Field(Ast.This, ((Ast.Id)n.lhs).nm);
            CodePack fieldGen = gen(field.obj, cinfo, env);
            ClassInfo objInfo = getClassInfo(field.obj, cinfo, env);
            int offset = objInfo.fieldOffset(field.nm);
            IR.Addr address = new IR.Addr(fieldGen.src, offset);        //forming an IR.Addr
            Ast.Type ftype = objInfo.fieldType(field.nm);
            code.add(new IR.Store(gen(ftype), address, rhsGen.src));
        }
      }
      else{
            CodePack fieldGen = gen(((Ast.Field)n.lhs).obj, cinfo, env);
            ClassInfo objInfo = getClassInfo(((Ast.Field)n.lhs).obj, cinfo, env);
            int offset = objInfo.fieldOffset(((Ast.Field)n.lhs).nm);
            IR.Addr address = new IR.Addr(fieldGen.src, offset);        //forming an IR.Addr
            Ast.Type ftype = objInfo.fieldType(((Ast.Field)n.lhs).nm);
            code.add(new IR.Store(gen(ftype), address, rhsGen.src));
      }
      return code;
  }
  ...
  static CodePack genCall(Ast.Exp obj, String name, Ast.Exp[] args, 
    ...
    List<IR.Src> argsList = new ArrayList<IR.Src>();
    List<IR.Inst> code = new ArrayList<IR.Inst>();
    ClassInfo classInfo = getClassInfo(obj, cinfo, env);
    Ast.Type rtype = null;
    ClassInfo baseInfo = classInfo.methodBaseClass(name);
    IR.Global callName = new IR.Global("_" + baseInfo.name + "_" + name);
    CodePack objGen = gen(obj, cinfo, env);
    code.addAll(objGen.code);
    IR.Addr address = new IR.Addr(objGen.src);
    argsList.add(address.base);
    IR.Temp temp = new IR.Temp();
    for(Ast.Exp e: args){
        CodePack argGen = gen(e, cinfo, env);
        argsList.add(argGen.src);
        code.addAll(argGen.code);
    }
    if(retFlag == true){
        rtype = classInfo.methodType(name);
        code.add(new IR.Call(callName, false, argsList, temp));
        return new CodePack(gen(rtype), temp, code);
    }
    else{
        code.add(new IR.Call(callName, false, argsList));
        return new CodePack(null, objGen.src, code);
    }
  }
  ...
  static List<IR.Inst> gen(Ast.If n, ClassInfo cinfo, Env env) throws Exception {
    ...
    CodePack condExpr = gen(n.cond, cinfo, env);  //generate codepack for cond
    code.addAll(condExpr.code);     //add all code from cond codepack
    IR.CJump inst = new IR.CJump(IR.ROP.EQ, condExpr.src, IR.FALSE, label1); 
    code.add(inst);
  ...
  static List<IR.Inst> gen(Ast.While n, ClassInfo cinfo, Env env) throws Exception {
    ...
    List<IR.Inst> code = new ArrayList<IR.Inst>();
    IR.Label label1 = new IR.Label();
    IR.Label label2 = new IR.Label();
    boolean s2Flag = false;
    code.add(new IR.LabelDec(label1));
    CodePack condExpr = gen(n.cond, cinfo, env);
    code.addAll(condExpr.code);
    code.add(new IR.CJump(IR.ROP.EQ, condExpr.src, IR.FALSE, label2));
    code.addAll(gen(n.s, cinfo, env));
    code.add(new IR.Jump(label1));
    code.add(new IR.LabelDec(label2));
    return code;
  ...
  static List<IR.Inst> gen(Ast.Print n, ClassInfo cinfo, Env env) throws Exception {
    ...
    if(n.arg == null){
        code.add(new IR.Call(new IR.Global("_printStr"), false, srcList));
    }
    else{
        ...
        if(arg.src instanceof IR.StrLit)
            code.add(new IR.Call(new IR.Global("_printStr"), false, srcList));
                ...
                code.add(new IR.Call(new IR.Global("_printBool"), false, srcList));
        else
            code.add(new IR.Call(new IR.Global("_printInt"), false, srcList));
        }
  ...
  static List<IR.Inst> gen(Ast.Return n, ClassInfo cinfo, Env env) throws Exception {
    ...
    if(n.val != null){
        ...
        code.add(new IR.Return(pack.src));
    }
    else
        code.add(new IR.Return());
    return code;
  ...
  static CodePack gen(Ast.NewObj n, ClassInfo cinfo, Env env) throws Exception {
    ...
    int size = classinfo.objSize;
    List<IR.Src> intList = new ArrayList<IR.Src>();
    if(size > 0){
        IR.Temp temp = new IR.Temp();
        IR.IntLit srcSize = new IR.IntLit(size);
        intList.add(srcSize);
        code.add(new IR.Call(new IR.Global("_malloc"), false, intList, temp));
        return new CodePack(IR.Type.PTR, temp, code);
    }
    else{
        IR.IntLit sizeZero = new IR.IntLit(0);
        return new CodePack(IR.Type.PTR, sizeZero, code); 
    }
  }
  ...
  static CodePack gen(Ast.Field n, ClassInfo cinfo, Env env) throws Exception {
     ...
     CodePack obj = gen(n.obj, cinfo, env);                             //base address
     ClassInfo classInfo = getClassInfo(n.obj, cinfo, env);   //getting the base ClassInfo
     int offset = classInfo.fieldOffset(n.nm);              //find out field's offset
     IR.Addr address = new IR.Addr(obj.src, offset);        //forming an IR.Addr
     Ast.Type ftype = classInfo.fieldType(n.nm); //classInfo.name);            //Find out field's type
     List<IR.Inst> code = new ArrayList<IR.Inst>();
     code.addAll(obj.code);
     IR.Temp temp = new IR.Temp();
     code.add(new IR.Load(gen(ftype), temp, address));
     return new CodePack(gen(ftype), temp, code);
  }
